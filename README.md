...
| `number_of_courses` | float64 | 10 | 0 | Number of courses the student is
  enrolled in. |
| `average_marks` | float64 | 1589 | 0 | Average marks obtained by the student
  across all courses. |
| `average_login` | float64 | 1024 | 0 | Average number of logins. |
| `weekend_login` | float64 | 291 | 0 | Average number of logins during
  weekends. |
| `weekday_login` | float64 | 948 | 0 | Average number of logins during
  weekdays. |
| `midnight_login` | float64 | 344 | 0 | Average number of logins during
  midnight hours. |
| `early_morning_login` | float64 | 291 | 0 | Average number of logins during
  early morning hours. |
| `late_morning_login` | float64 | 291 | 0 | Average number of logins during
  late morning hours. |
| `afternoon_login` | float64 | 344 | 0 | Average number of logins during
  afternoon hours. |
| `evening_login` | float64 | 102 | 0 | Average number of logins during
  evening hours. |
| `night_login` | float64 | 102 | 0 | Average number of logins during night
  hours. |
| `no_of_viewed_courses` | float64 | 1148 | 0 | Average number of courses
  viewed. |
| `no_of_attendance_taken` | float64 | 102 | 0 | Average number of attendances
  taken. |
| `no_of_all_files_downloaded` | float64 | 102 | 0 | Average number of files
| `number_of_courses` | float64 | 10 | 0 | Number of courses the student is
  enrolled in. |
| `total_marks` | float64 | 1589 | 0 | Total marks obtained by the student
  across all courses. |
  submitted. |
| `no_of_forum_created` | float64 | 102 | 0 | Average number of forum posts
  created. |
| `number_of_quizzes` | float64 | 102 | 0 | Average number of quizzes
  available. |
| `no_of_quizzes_completed` | float64 | 102 | 0 | Average number of quizzes
| `formatted agreed mark` | object | 1589 | 0 | Formatted agreed mark for the
  course. |
| `no_of_quizzes_attempt` | float64 | 102 | 0 | Average number of quizzes
  attempted. |
| `average_login` | float64 | 1024 | 0 | Average number of logins. |
| `weekend_login` | float64 | 291 | 0 | Average number of logins during weekends.|

- The `SED_Student_log.csv` is a large file and will require significant
  processing to extract meaningful features for engagement. This will involve
- The `userid` and `courseid` columns will be crucial for joining the log data
  with the grade and activity summary files.
| `midnight_login` | float64 | 344 | 0 | Average number of logins / midnight hours|
| `early_morning_login` | float64 | 291 | 0 | Average logins/ early morning hours|

1. **Feature Engineering from `SED_Student_log.csv`:** Transform raw log data
   into features representing student interaction patterns (e.g., total logins,
   time spent, forum activity counts, resource access frequency).
2. **Data Integration:** Join the engineered features from `SED_Student_log.csv`
   with `Student_activity_summary.csv`, `Student_grade_aggregated.csv`, and
   `Student_grade_detailed.csv` using `userid` and `courseid`.
3. **Final Cleaning and Validation:** Address any remaining missing values,
   outliers, and inconsistencies in the combined dataset.
4. **Final Data Dictionary:** Create a comprehensive data dictionary for the
   final, cleaned, and integrated dataset.
| `no_of_viewed_courses` | float64 | 1148 | 0 | Average number of courses viewed.|
| `no_of_attendance_taken` | float64 | 102 | 0 | Average number of attendances taken.|
| `no_of_all_files_downloaded` | float64 | 102 | 0 | Average number of files downloaded.|
| `no_of_assignments` | float64 | 102 | 0 | Average number of assignments submitted.|
| `no_of_forum_created` | float64 | 102 | 0 | Average number of forum posts created.|
| `number_of_quizzes` | float64 | 102 | 0 | Average number of quizzes available.|
| `no_of_quizzes_completed` | float64 | 102 | 0 | Average number of quizzes completed.|
| `no_of_quizzes_attempt` | float64 | 102 | 0 | Average number of quizzes attempted.|
...
| `total_marks` | float64 | 1589 | 0 | Total obtained by the student across all courses.|
